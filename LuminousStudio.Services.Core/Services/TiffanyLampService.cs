namespace LuminousStudio.Services.Core.Services
{
    ...
    public class TiffanyLampService : ITiffanyLampService
    {
        ...
        public TiffanyLampService(ITiffanyLampRepository tiffanyLampRepository)
        {
            _tiffanyLampRepository = tiffanyLampRepository;
        }

        public async Task<bool> CreateAsync(string name, Guid manufacturerId, Guid lampStyleId,
            string picture, int quantity, decimal price, decimal discount)
        {
            ...
            await _tiffanyLampRepository.AddAsync(item);
            return true;
        }
        ...
        public async Task<List<TiffanyLamp>> GetTiffanyLampsAsync(
            string searchStringLampStyleName, string searchStringManufacturerName)
        {
            IQueryable<TiffanyLamp> query = _tiffanyLampRepository
                .GetAllAttached()
                .Include(x => x.Manufacturer)
                .Include(x => x.LampStyle);

            if (!string.IsNullOrEmpty(searchStringLampStyleName) &&
                !string.IsNullOrEmpty(searchStringManufacturerName))
            {
                query = query.Where(x =>
        ...
        public async Task<bool> RemoveByIdAsync(Guid tiffanyLampId)
        {
            ...
            return await _tiffanyLampRepository.DeleteAsync(tiffanyLamp);
        }

        public async Task<bool> UpdateAsync(Guid tiffanyLampId, string name, Guid manufacturerId,
            Guid lampStyleId, string picture, int quantity, decimal price, decimal discount)
        {
            ...
            return await _tiffanyLampRepository.UpdateAsync(tiffanyLamp);
        }
    }
}